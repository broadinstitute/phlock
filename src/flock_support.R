...
flock.run <- function(inputs, task_script_name, gather_script_name=NULL, flock_common_state=NULL, gather_params=NULL, script_path=NULL, flock_run_dir=NULL) {
  ...
  for(job.index in 1:length(inputs)) {
    ...
    save(flock_run_dir, flock_job_dir, flock_input_file, flock_output_file, flock_script_name, flock_per_task_state, flock_completion_file, file=flock_input_file)
