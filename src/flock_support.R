...
flock.run <- function(inputs, task_script_name, gather_script_name=NULL, shared_params=NULL, gather_params=NULL, script_path=NULL, run_dir=NULL) {
  ...
  for(job.index in 1:length(inputs)) {
    ...
    job_dir = paste(run_dir, '/', task.dir, '/', job.subdir, sep='');
    ...
    submit_command('1', paste(job.subdir, '/task.sh', sep=''), paste('exec R --vanilla --args ', shared_params_file, ' ', input_file, ' < ', script_path, '/execute_task.R', sep=''))
